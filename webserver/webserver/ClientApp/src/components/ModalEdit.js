...
export class ModalEdit extends React.Component { //класс модального окна редактирования фильма
    constructor(props) {
        super(props);
        this.state = {
            modal: false, name: this.props.film.name, timing: this.props.film.timing, id_genre: this.props.film.id_genre,
            id_country: this.props.film.id_country, year: this.props.film.year, age: this.props.film.age, original: this.props.film.original,
            description: this.props.film.description, poster: this.props.film.poster, genres: [], countries: []
        };
    ...
    handleSubmit(e) { //обработка нажатия на кнопку "изменить"
        ...
        if (!filmName || !filmDescription || !filmTiming || !filmGenre || !filmCountry || !filmAge || !filmYear) {
            return;
        }
        this.props.onFilmSubmit({ id: filmId, name: filmName, description: filmDescription, timing: filmTiming, id_genre: filmGenre, id_country: filmCountry, poster: filmPoster, year: filmYear, age: filmAge, original: filmOriginal });
        this.toggle();
    }
    // загрузка данных
    loadData() {
